package net.sethyn.handcrafted.mixin;
...
import net.sethyn.handcrafted.HandcraftedModPlayerAnimationAPI;
...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (entityIn instanceof Player player_)
			...
			return;
		HandcraftedModPlayerAnimationAPI.PlayerAnimation animation = HandcraftedModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (ageInTicks <= 0)
			return;
		if (!master.equals("handcrafted")) {
			if (!HandcraftedModPlayerAnimationAPI.animations.isEmpty())
				HandcraftedModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			HandcraftedModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		HandcraftedModPlayerAnimationAPI.PlayerAnimation animation = HandcraftedModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = HandcraftedModPlayerAnimationAPI.animations.get(playingAnimation);
			if (animation == null) {
				HandcraftedMod.LOGGER.info("Attepted to play null animation " + playingAnimation + ", did animations fail to load?");
				return;
			}
			HandcraftedModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, HandcraftedModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			HandcraftedModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = HandcraftedModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress, player);
			if (rotation != null) {
			...
			Vec3 position = HandcraftedModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress, player);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = HandcraftedModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress, player);
			if (scale != null) {
